...
`src/rpc-child-extension.ts` is the source-mode RPC child entry. It installs the same headless profile as the canonical entry without importing classic UI modules. `src/sumo-tui/rpc/spawn-child.mjs` selects it for forced source mode; `src/extension-entry.ts` selects it when an RPC child falls back from a missing or stale bundle. Keep the shared profile in `src/extension-core.ts` and the source entry shallow.
