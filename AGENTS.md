...
- Footer right zone: landscape (sidebar visible) paints the palette keybind (`CTRL+/ · COMMANDS`) with context/window + cost sidebar-owned; portrait keeps context/window + cost in the footer and the hint row (project/branch left, keybind right). Landscape has no hint row — the freed row returns to the transcript.
