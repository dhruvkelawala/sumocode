...
Fixtures and detached worktrees are deleted after collection. `--keep-fixture` explicitly retains the final generated state for each arm and prints its temporary path; worktrees are still removed. Reports are retained in `--out`, or in the private temporary directory printed when `--out` is omitted.
...
- **RPC child ready**: launch to `SumoRpcClient.start()` completion.
...
A directional verdict requires zero failed samples and non-overlapping intervals. Any missing event, failed process, interval overlap, or conflicting metric direction makes overall startup **INCONCLUSIVE**. These are report-only measurements, not CI wall-clock gates.
...
Raw JSONL diagnostics, stderr, and PTY bytes remain process-local. Reports whitelist only numeric phase values, fixed failure categories/event names, exact SHAs, source mode, fixed sanitized flags, fixture count, and coarse runtime metadata (platform, architecture, Node version, CPU count). They never serialize absolute home paths, prompts, environment values, provider/model names, session IDs, or extension inventory.
