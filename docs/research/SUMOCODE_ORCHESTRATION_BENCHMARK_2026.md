...
8. **Notifications should be policy-driven and deduplicated.** Terminal indicators are for progress; desktop notifications should fire on approval needed, failure/lost worker, and completion while unfocused—not every tool call. Include parent/task identity and a focus action. Claude’s Notification hook is the best direct analogue.
...
- terminal-pane layout balancing, resource metrics and remote/cloud execution backends.
...
- **Portable event contracts:** Claude hooks are strong but product-specific. A stable, typed event journal connecting terminal UI, terminal-host notifications, logs and external automation would differentiate SumoCode.
