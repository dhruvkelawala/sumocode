...
**Effect v4 replacement — verified against the real Node implementation.** `effect/unstable/process` declares the shape: `ChildProcess.make(cmd, args, { detached, env, extendEnv })` (`ChildProcess.ts:603`), `ChildProcessSpawner.spawn` returning a scoped `ChildProcessHandle` (`ChildProcessSpawner.ts:79-196`) with `exitCode`, `isRunning`, `stdin` sink, `stdout`/`stderr`/`all` streams, `kill(options)`, `unref`. `KillOptions` (`ChildProcess.ts:242-254`) is exactly `killSignal` + `forceKillAfter`.

The implementation lives in `@effect/platform-node` → `@effect/platform-node-shared`, both at `4.0.0-rc.112` (present in the local npm cache at `/Volumes/SumoDeus NVMe/.npm-cache/@effect/platform-node-shared/4.0.0-335d1bb637e03e00@@@1`). Reading `src/NodeChildProcessSpawner.ts` there, it matches SumoCode's hand-rolled semantics **almost line for line**:

- `killProcessGroup` (`:362-384`) is `globalThis.process.kill(-childProcess.pid!, signal)` on POSIX and `taskkill` on Windows — identical to `backend-pi.ts:425-433` and `process-tree.ts:288`.
- `handle.kill` (`:546-556`) is `killProcessGroup(...).pipe(Effect.catch(() => killProcess(...)), Effect.andThen(Deferred.await(exitSignal)))` — group first, single-pid fallback, then **wait for actual exit**. That `Effect.andThen(Deferred.await(...))` is what makes `withTimeout` (`:416-435`, `Effect.timeoutOrElse(… , orElse: SIGKILL)`) a real TERM→grace→KILL escalation rather than a timeout on an instantaneous syscall.
- `internal/nodeChildProcessSpawner.ts:9` — `detached` defaults to `true` on non-win32, so the process group always exists and the negative-pid kill is always valid.
- `Effect.acquireRelease` (`:484-511`) makes the scope finalizer kill the group, and `:498-500` correctly *skips* the kill when the handle was `unref`'d — matching `task-manager.ts:716`'s `child.unref()` intent.

**So the earlier "do not adopt" verdict is too strong. Split the decision:**

- **`backend-pi.ts` / `native-task-tool.ts` (headless children): adopt.** These are spawn → stream stdout → kill → forget. No cross-restart recovery, no durable identity. `ChildProcessSpawner` deletes both copies of `attachAbortSignal`, both `signalGroup`s, the manual `dispose()`/listener removal (`backend-pi.ts:658-662`), and turns the `JsonLineDecoder` plumbing into `handle.stdout |> Stream.decodeText |> Stream.splitLines`.
- **`background-tasks` (durable terminals): do not adopt.** Three blockers stand:
  1. `ChildProcessHandle` exposes `pid` and nothing else. SumoCode's durable model needs PID + `ps lstart` + command fingerprint (`process-tree.ts:199-215`) and member anchors with ABA bracketing (`:318-329`) to survive a manager restart. A handle cannot be reconstituted from disk. This is decisive.
  2. `NodeChildProcessSpawner.ts:514-518` installs an `exit` listener that **auto-kills the process group on any non-zero exit**. SumoCode's terminals record natural exit codes and read durable exit markers (`task-manager.ts:863`, `:906-925`); an unrequested group-kill on non-zero exit is a behaviour change against `prefers a concurrent natural exit marker over cancellation after stop disposition` (`task-manager.test.ts:2474`).
  3. Dependency weight: `@effect/platform-node` pulls `@effect/platform-node-shared`, `mime`, and `undici` into a repo with three production dependencies and a `packages: "external"` bundle (`scripts/build-extension.mjs:24`).

**Recommendation:** introduce a `ChildSupervisor` `Context.Service`. Implement it with `ChildProcessSpawner` for the headless-subagent path (slice 5a) and keep the plain-Node/`process-tree.ts` implementation for durable terminals (slice 5b). One tag, two layers, no duplicated `attachAbortSignal`.
...
**LOC affected.** ~120 (dedupe) / ~300 (if `waitForTreeEmpty` also moves to `Effect.repeat`). **Effort: M. Risk: MED** — adoption on the headless path must be gated on `pnpm test:integration`'s zero-survivor audit, since it changes who sends the signal.
...
**Split.** `@effect/platform-node`'s `NodeChildProcessSpawner` (verified at rc.112 in the local npm cache) reproduces SumoCode's group-kill + TERM→grace→KILL semantics faithfully — see A6. Adopt it for **headless subagent children** (`backend-pi.ts`, `native-task-tool.ts`), where the child is spawned, streamed, killed, and forgotten within one scope.

**Do not adopt it for `background-tasks`.** `ChildProcessHandle` carries no durable identity anchor, so it cannot express "verify this pid is still the same process after a manager restart" — the foundation of `process-tree.ts:181-215` and `:318-329`. It also auto-kills the group on non-zero exit (`NodeChildProcessSpawner.ts:514-518`), which conflicts with natural-exit-marker precedence. Keep `node:child_process` + `process-tree.ts` behind the same `ChildSupervisor` tag with a different layer.

**Dependency note.** Adopting it adds `@effect/platform-node` + `@effect/platform-node-shared` (+ transitively `mime`, `undici`) on top of `effect`. Against a repo with three production dependencies and `packages: "external"`, that is the single largest weight increase in this campaign — weigh it against deleting ~120 lines of duplicated signal handling.
...
| **5** | **A6** — introduce `ChildSupervisor` with two layers. **5a:** headless children (`backend-pi.ts`, `native-task-tool.ts`) onto `ChildProcessSpawner` from `@effect/platform-node` — deletes both `attachAbortSignal` copies and both `signalGroup`s. **5b:** durable terminals keep the plain-Node/`process-tree.ts` layer; move `waitForTreeEmpty` to `Effect.repeat`. | `src/subagents/backend-pi.ts`, `src/native-task-tool.ts`, `src/background-tasks/process-tree.ts` | `backend-pi.test.ts`, `process-tree.test.ts`, **plus `pnpm test:integration` zero-survivor audit (mandatory — 5a changes who sends the signal)** | 1, 3 |
...
**5. Startup and dependency weight, against a budget that was just fought for.** Plan 117 moved editor-ready from 549 → 182 ms and command-ready from 2,199 → 1,008 ms. `dist/Effect.js` is 251 kB; the realistic lifecycle graph is ≈ 800 kB–1 MB of externally-resolved ESM. `effect` also carries two runtime `dependencies` (`fast-check`, `msgpackr`) — verified not statically imported by `Schema`/`Effect` (`internal/schema/toArbitrary.ts:13` is `import type`), but they *are* installed for every consumer of a repo that is public MIT with a deliberately thin dependency list (currently three prod deps). Slice 5a compounds this: `@effect/platform-node` + `@effect/platform-node-shared` + `mime` + `undici`. That is 1 → 7 production dependencies to delete ~120 lines of signal handling; it is a defensible trade only if the extension bundle moves off `packages: "external"` for these, which is its own decision. Lazy-import discipline (§4.1) is the mitigation, but it is discipline, not a guarantee — one careless top-level `import { Effect } from "effect"` in `src/extension.ts`'s load order regresses the whole campaign silently.
