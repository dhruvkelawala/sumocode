...
## Bounded continuation from `ef0d7c7c`: headless launch admission only

The coordinating parent approved implementation continuation from clean `ef0d7c7c952683e58d90b89bbf354cb9981b9ebf`, with sa83's 46-test registry CLEAN and sa74's fixture CLEAN as supplied review evidence. This does not assert new reviewer acceptance or production crash proof. Heavy verification remains assigned elsewhere (sa87). Plan112 and the index remain in progress; historical evidence limits above are unchanged.

### Implemented seam and deliberate narrowing

The full retained process owner is larger than this first coherent slice. `src/subagents/retained-supervisor.ts` now implements **launch admission only**, callable by the future production entry, not a fake supervisor or a Vitest runtime adapter. `createRetainedHeadlessLaunchGate` creates `starting`, acquires the existing registry lease, binds the current dedicated supervisor identity/member birth to its writer, and supplies the real Pi backend's optional synchronous launch checks. It does not own a running backend or publish results yet; **do not enable production retention with this module alone**.

- `beforeSpawn` verifies the supervisor and CAS-fences revision/generation/token/PID/birth/live expiry immediately before the backend spawn call. `beforePrompt` rechecks authority, captures/persists the child's dedicated-group identity/member anchors while still `starting`, verifies both identities, then persists `running` before returning to the stdin write. All registry callbacks decide metadata only; OS inspection and process/pipe effects remain outside transactions. Local one-shot state prevents retries; disk remains authority.
- `src/subagents/backend-pi.ts` adds only an optional `launchGate`. Gated launches wait for the real `spawn` event before authorizing the existing stdin prompt write/end, install the existing parser/close/error handlers first, expose a `ready` promise, and refuse a second event subscription. Main prompt bytes stay out of argv; model/bootstrap/tool/role selection remains unchanged. Gated launches reject a non-absolute Pi selection instead of falling back to PATH. Plan108's resolver remains the sole selector; no Node/Bun/executable selection was added.
- Refusal rejects `ready` (or synchronously rejects subscription before spawn) without releasing/closing stdin, inventing settlement, or signalling. The future owner must keep that blocked backend handle for verified accounting/cleanup. Existing ungated launches retain their immediate prompt and disposal behavior. `manager.ts`, `index.ts`, `backend-pane.ts`, registry APIs, task wrapper, native/build entry files and all delivery/Activity files are unchanged.

### Proof limits and next kernel work

The tests use real private registry files and the production Pi parser/gate with fake child-process/OS operations. They verify starting-at-spawn, anchors/running-at-stdin-write, one subscription, rejection before spawn/release for stale revision, expiry, dead supervisor or corrupt records, expiry during OS capture, and preservation of captured anchors on a final identity refusal. They are **not process crash tests**, observer adoption, or an independently running supervisor.

Unfinished gates must be closed before packaging enables launches:

1. A production process owner must retain exactly one backend handle/parser, multiplex replaceable read-only observers without resubscribing, renew its writer lease, and own bounded private journal/result/manifest persistence. No result/manifest pointer or settlement event may precede durable validated bounded artifacts. Corrupt replay must fail closed.
2. There is still an unproved spawn-to-child-anchor window. A capture/lease/disk failure can leave `starting` with no child evidence and a held pipe; a crash can lose that handle. The entry/launch accounting protocol must durably account for every fresh spawn and supervisor before release and prove zero untracked children. A bare PID must never become reconnection or cleanup authority. The first `spawn` snapshot's stability across Pi's process-title/startup changes also needs real proof.
3. `running` here is a release **intent**, not provider acceptance: a crash between the final CAS and stdin write, or EPIPE after write, is uncertain. Recovery must classify that interleaving as ambiguous, not replay the prompt. Dead supervisors must become lost/ambiguous, never PID-based pipe adoption. These classifications are not implemented by this launch-only function.
4. Control remains unimplemented. Fence request revision/token/birth/live lease at each actual send/signal/close effect; recheck after async acknowledgement and report AMBIGUOUS on expiry/interleaving, never retry control. Existing backend abort/protocol-error/escalation paths also need retained-owner fencing before enablement; this slice deliberately does not retrofit their signal policy.
5. Visible support needs its own private wrapper wait gate **before launcher/work**, a task directory known to the registry before pane creation, and verified pane-associated wrapper/child tree anchors before release. Do not treat the current PID-only started marker or a pane ID as authority.
6. Unit cut points for settling/post-manifest, durable bounded output/corruption, supervisor death classification, asynchronous control expiry and observer replacement remain next slices. Then obtain the heavy grant for real starting/pre-release/running/settling/post-manifest crashes and supervisor death before release/while running. Adoption/delivery remain strictly after that production gate.

### Serial entry integration handoff (design only)

- **Native owner: Plan111, `src/native/main.ts`.** Its `runTerminalRunnerRole()` near line 93 and dispatch near line 1063 establish the correct pattern. Add a private `--sumocode-subagent-supervisor` role before any launcher/host dispatch, importing a SumoCode-owned production entry by a literal module path into the **same compiled binary**. Proposed entry path: `src/subagents/retained-supervisor-entry.ts`, with a narrow `runRetainedSupervisor` function, not implemented yet. Pass only validated private launch-record paths/nonce on argv; prompt/control/result content stays private. Do not route through Pi host/client constructors, private hooks, or ordinary user CLI parsing.
- **Native build owner: Plan87, `scripts/build-native.mjs`.** Its existing `src/native/main.ts` compile root should embed the literal production entry; verify bundle imports/externals, executable/model/tool provenance and native role tests after serial merge. No separate Bun installation, sidecar runtime or Pi patch is proposed. Neither owned file was edited.
- **Source counterpart, pending parent integration review:** proposed `src/subagents/retained-supervisor-entry.mjs` uses the package-relative `createJiti(import.meta.url, { moduleCache: true, tryNative: false })` pattern from `sumo-rpc-host.js#importSourceHost` to call the same TS entry. Do not import or boot the RPC host itself. `task-manager.ts#resolveTerminalRunnerInvocation` is a role-selection precedent, **not** permission to copy its unconditional source `process.execPath`: source launch must prove a Node parent (`process.versions.node` and no `process.versions.bun`) and retain its verified absolute executable provenance. Native selects the already-selected absolute SumoCode binary/private role; unrecognized Bun/source provenance fails closed, with no PATH Node/Pi fallback. Validate the owned package root/bootstrap rather than resolving executable code from the task cwd. No source bootstrap or packaging helper is implemented in this slice.

### Lightweight verification and review-ready gate

- RED: the first backend test failed because no gate ran; the first kernel test failed on the absent module. The stale pre-spawn test then exposed an unresolved `ready` promise, and the PATH-Pi test exposed a real spawn attempt before provenance refusal. Both corrected and rerun green.
- `pnpm vitest run src/subagents/retained-supervisor.test.ts src/subagents/backend-pi.test.ts src/subagents/registry.test.ts --maxWorkers=1 --fileParallelism=false`: **110 passed** (13 launch-admission, 51 Pi backend, 46 registry). Final serial test/static log: `/tmp/sumocode-plan112-launch-2nQF9q/lightweight.log`.
- `pnpm exec tsc --noEmit && pnpm build`, focused Oxlint over the four changed TS files, and `git diff --check`: passed. An initial mock literal-inference type error and focused lint findings were corrected before this final result.
- No install, PTY, process-crash, full unit/integration, bundle, native or visual run; no deletion, push, golden promotion or private099 changes. Full production gate remains blocked on the work listed above and heavy permission.
- Review-ready contract: bundled `review-ready/contract.md`. Trace: private starting record → writer/supervisor binding → spawn fence → one backend subscription → durable child anchors → release-intent CAS → stdin. Caller-knowledge: the future entry supplies identities and owns the handle, not registry locking. Deletion: removing the gate would spread launch ordering/fencing into entries. Ownership: disk policy stays in the registry; launch policy in the kernel; pipe/parser mechanics in the backend. Test-surface: public registry/gate/backend factories, no Pi internals. Simplification kept a two-check launch interface, reused registry/process operations, and added no framework or generic CLI. Exceptions: all explicit launch-only/proof/enablement limits above; ready for review of this bounded seam, **not** production retention acceptance.
...
## Slice 2 final sender decision (supersedes the receive-contract blocker)

The accepted counterexamples remain facts: Pi offers no idempotent insertion contract and host event filtering cannot veto admitted messages. The final decision keeps the existing transport and defines exact-once at the **sender**, with an explicit ambiguous non-atomic window. The plan's exact-once criterion is met for deterministic sender paths with this documented boundary; `sent` is synchronous submission return, **not proof of Pi conversation insertion**, queue draining, or asynchronous acceptance.

- Settled records now publish `undelivered`. The manager's delivery owner performs generation/identity/head/revision-fenced registry CAS to `sending { completionId, controllerGeneration, at }`, calls the existing synchronous `pi.sendMessage` once without an intervening await, then CAS-marks `sent { completionId }`. Ordinary persistence-writer transitions cannot edit delivery after settlement.
- A successor never replays `sent`. It sends `undelivered`; previous-generation `sending` becomes `delivery-uncertain`, never a completion replay. It submits one visible `subagent-delivery-uncertain` message: `delivery of <id> uncertain; result manifest available at <path>; use inspect`. A distinct custom type keeps this notice out of Activity's completion-card deduplication path.
- The notice has its own durable send slot and stable `<completionId>:uncertain` ID. If that slot's send/return is also lost, its successor marks the notice uncertain and does not replay it or recursively generate more notices. This is the **same non-atomic boundary**, not an exactly-once visible-insertion promise under repeated crashes. Lost CAS returns and synchronous send exceptions leave durable evidence authoritative.
- The existing cooperative replacement detaches the outgoing manager before reservation, and all delivery CAS operations reject stale authority before and after metadata decisions. A successfully settled retained supervisor now permits that same transfer while its existing child/supervisor/lease checks hold; no dead parser or anchor is revived. Corrupt delivery records become blocked views, with zero sends and no repair. Supervisor/anchor loss retains existing lost/ambiguous classification and zero replay.
- No automatic deletion, lease relaxation, default retained launch activation, transport, Pi patch, receiver protocol, or dependency was added. The dormant experimental pending/claimed schema is not silently promoted to sender authority; such old records fail closed and remain on disk. Legacy non-retained children keep their existing delivery behavior and unsupported-replacement boundary.

Acceptance uses manager/lifecycle and registry interfaces with fake Pi/backend/process boundaries and real private files: settled-before-replacement for both backends; sent-before-replacement; crash before call, after admission publication but before CAS return, and before synchronous send return; one notice through further replacement; a lost notice return; stale controller CAS before/after successor grant; concurrent same-revision admission; writer-bypass refusal; corrupt state/id/generation/time; corruption during replacement; supervisor/anchor loss with undelivered or sending state. Every new settled-result case asserts readable result and manifest artifacts. These are deterministic source tests, not process-crash or visual capture evidence.
...
### Spawn accounting and census checkpoint (after `437cd8bf`)

Fake matrix: **38/38**, zero skipped. All subagent units: **658/658** (23 files); adjacent process-tree/census units: **18/18**. Project tsc/build, strict integration-file tsc and scoped lint pass. No real-mode or heavy command ran.

The shared launch gate now persists an immutable launch intent before calling either backend's spawn. Explicit null means not admitted; an intent without a persisted child means `launched-unknown`, even when the census finds no process. Absent legacy intent remains unknown. The headless anchor carries that intent nonce in its stable command. A successor can observe matching nonce hints but cannot turn them into new signal identities, reopen pipes, retry launch, or declare cleanup complete. This closes durable accounting, not atomic spawn/identity publication: a pre-publication orphan may still require manual inspection.

`censusRetained` enumerates the installation registry and uses `ProcessTreeOperations.census` to compare retained anchors/children against live groups. `systemProcessTree` supplies the POSIX adapter: one bounded absolute `/bin/ps` query, strict rows/unique PIDs/observer-presence checks, no raw command output retained. Failure, partial output, unsupported platform or missing enumeration is unknown. Empty requires both a complete census with no child-group members and the existing conservative tree-empty check. Startup also writes recovery observations for records that never acquired a control lease. Census never signals, captures replacement cleanup anchors, or deletes records/artifacts.
