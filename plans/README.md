...
| 095 | [Honest, settlement-aware visible steering ACKs](095-truthful-visible-steering-acks.md) | P1 | M | M1 | 092 | DONE — `3df8c7b`, review fixes `18b35b4`, Codex P2 repairs `eb76e50` (consumed acks resolve at settle/interrupt) + `0de79e8` (submitted ownership spans watcher recreation); 164/164 focused; 111/111 integration; typecheck/build/lint green; bundle rebuilt deterministic; full unit one-worker reruns flake only in untouched files (same set fails at base `4bcb57c`; each passes in isolation, host-actions 66/66), so default parallel remains load-sensitive from pre-existing timing flakes in untouched files; CI pending | [#389](https://github.com/dhruvkelawala/sumocode/issues/389) |
