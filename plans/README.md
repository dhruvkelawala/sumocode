...
| 085 | [/roles palette UX rebuild](085-roles-palette-ux.md) | P2 | M | 083 | DOGFOOD — implementation intentionally revised after operator feedback to one-level role drill-in; registry picker + RPC search/caret verified; final status depends on 083 acceptance repair and updated PR |
...
| 083 | [Role-based fire-and-forget subagents](083-role-based-async-subagents.md) | P2 | L | — | ACCEPTANCE REPAIR — backend roles/queue/delivery live-verified; RPC status strip fixed to publish string lines, PTY regression added, visible Herdr children now split beside parent; unit + targeted integration + visual CI green; full local integration suite still has unrelated/cascading timeout failures, and operator reload/live visual approval remains before DONE; [PR #372](https://github.com/dhruvkelawala/sumocode/pull/372) is stale at `3b09902` |
