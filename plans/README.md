...
| 095 | [Honest, settlement-aware visible steering ACKs](095-truthful-visible-steering-acks.md) | P1 | M | M1 | 092 | DONE — `3df8c7b`, review fixes `18b35b4`, Codex P2 repairs `eb76e50` (consumed acks resolve at settle/interrupt) + `0de79e8` (submitted ownership spans watcher recreation), Opus should-fix repairs `c497f9f` (process-wide globalThis ownership, ENOENT ack completion, single canonicalization; snapshot test seam); 143/143 focused (43 task-mode + 23 backend-pane + 53 manager + 24 tools); 2,529/2,529 full unit one-worker; 111/111 integration; typecheck/build/lint green; bundle rebuilt deterministic twice, clean status; stable load: one-worker and serial runs are fully green, default parallel remains load-sensitive from pre-existing timing flakes in untouched files (same set fails at base `4bcb57c`; each passes in isolation, host-actions 66/66); CI pending | [#389](https://github.com/dhruvkelawala/sumocode/issues/389) |
