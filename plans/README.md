...
|---|---|---|---|---|---|---|---|
| 091 | [Isolate extension-install tests](091-isolate-extension-install-tests.md) | P0 | S | M1 | — | BLOCKED — gate policy reconciled; exact-head focused/type/build/lint/full one-worker rerun required after final restack | [#385](https://github.com/dhruvkelawala/sumocode/issues/385) |
| 092 | [Credential-safe PTY harness — sanitized public brief](issues/092.md) | P0 | M | M1 | — | DONE — `8441296`; 21/21 focused; 2,514/2,514 unit (2 workers); 111/111 integration; typecheck/build/lint green; CI pending | [#386](https://github.com/dhruvkelawala/sumocode/issues/386) |
| 095 | [Honest, settlement-aware visible steering ACKs](095-truthful-visible-steering-acks.md) | P1 | M | M1 | 092 | DONE — `3df8c7b`, review fixes `18b35b4`, Codex P2 repairs `eb76e50` (consumed acks resolve at settle/interrupt) + `0de79e8` (submitted ownership spans watcher recreation), Opus should-fix repairs `c497f9f` (process-wide globalThis ownership, ENOENT ack completion, single canonicalization; snapshot test seam), contract/hygiene fixes `dded8cd` (explicit test-named submitted-control registry reset seam called in the control-watcher afterEach; Plan 095 scope/verification reconciliation in this commit); execution focused 143/143 (43 task-mode + 23 backend-pane + 53 manager + 24 tools), final hygiene focused 44/44 task-mode; 2,530/2,530 full unit one-worker (default parallel is informational/local load-sensitive; CI authoritative); 111/111 integration (one earlier local run hit 4 timing-flake failures in untouched specs; immediate rerun fully green; runtime bundle `.mjs` byte-identical to HEAD); typecheck/build/lint green; bundle rebuilt deterministic, `.mjs` unchanged (test-only seam tree-shakes), clean status; stable load: one-worker and serial runs are fully green, default parallel remains load-sensitive from pre-existing timing flakes in untouched files (base is likewise not green under default parallel; failing files vary by run and are untouched; each passes in isolation, host-actions 66/66); CI pending | [#389](https://github.com/dhruvkelawala/sumocode/issues/389) |
