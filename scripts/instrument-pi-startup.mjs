...
// direct runs patch the installed package while native builds pass a detached copy.
import { cpSync, lstatSync, readFileSync, readdirSync, realpathSync, statSync, unlinkSync, writeFileSync } from "node:fs";
import { dirname, join, resolve, sep } from "node:path";
import { pathToFileURL } from "node:url";

const root = resolve(import.meta.dirname, "..");

function installedPiDist() {
	return dirname(realpathSync(join(root, "node_modules/@earendil-works/pi-coding-agent/dist/index.js")));
}

export function instrumentPiStartup(piDist = installedPiDist()) {
	function detachDirectories(relativeDirectory) {
	...
	function replaceOnce(relativePath, needle, replacement) {
		detachDirectories(dirname(relativePath));
		...
		if (lstatSync(path).isSymbolicLink() || stats.nlink > 1) {
			unlinkSync(path);
			writeFileSync(path, contents, { mode: stats.mode });
		} else {
	...
	detachDirectories("bundle/chunks");
...
if (import.meta.url === entryUrl) instrumentPiStartup();
