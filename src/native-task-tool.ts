...
const attachAbortSignal = (
	...
	const terminate = () => {
		// A pid-less handle owns no child (spawn failed or has not completed), so
		// there is nothing to signal or escalate against.
		if (closed || forceKill || proc.pid == null) return;
		proc.kill("SIGTERM");
