...
export function buildSubagentPromptGuidelines(roles: readonly SubagentRole[]): string[] {
	return [
		...
		"All children have their own context, cannot see this conversation, and only know what your prompt carries; prompts must be self-contained with objective, paths, constraints, expected output, and stop conditions.",
		"Use subagent_send to steer a running visible child; success means the child runtime consumed the control and synchronously submitted it to Pi, and Pi exposes no post-acceptance acknowledgement. It does not prove the text was delivered as a Pi steering message or accepted into a model turn, and it is not typed into its terminal. running headless children cannot receive input.",
		"prefer subagent_reply over respawning with re-pasted context when iterating on a settled headless child's output; visible children take subagent_send while open.",
		...
		"the role's resolved model runs that child, and for the work the child does itself the child's model IS the implementer's model — do not assume your own model or a reviewer will write the code; pick the role deliberately, and treat an explicit `model` as the only override.",
...
export const SUBAGENT_PROMPT_SNIPPET = "Spawn, steer, reply to, check, wait for, cancel, and list headless or visible subagents with self-contained prompts.";
...
export const SUBAGENT_TOOL_DESCRIPTIONS = {
	...
	reply: "Continue a settled headless subagent's conversation in a new child that resumes its session — full prior context, no re-prompting. Visible children converse live via subagent_send instead.",
