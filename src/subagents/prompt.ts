...
export function buildSubagentPromptGuidelines(roles: readonly SubagentRole[]): string[] {
	return [
		...
		"headless children cannot spawn — they run without the extension's toolset; a visible child of an unnarrowed parent keeps the full toolset and still can. either way the child's model IS the implementer's model: pick the role whose resolved model you want writing the code; an explicit `model` is the only override.",
