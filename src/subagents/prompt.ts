...
export function buildSubagentPromptGuidelines(roles: readonly SubagentRole[]): string[] {
	return [
		...
		"headless children and tool-narrowed visible children cannot spawn: the narrowed set strips the extension's tools. an implement role carries no tool narrowing of its own, so from an unnarrowed parent its visible child keeps the subagent tools and can delegate again. for the work that child does itself, the child's model IS the implementer's model — pick it deliberately; an explicit `model` is the only override.",
