...
export async function runRpcHost(options: RpcHostMainOptions = {}): Promise<number> {
	...
	try {
		...
		// first-frame path. start() transfers signal ownership synchronously once
		// its child lifecycle listeners are attached.
		const childStart = client.start(adoptChildAndArmHostSignals);
