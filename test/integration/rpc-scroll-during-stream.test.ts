...
describe("sumocode RPC scroll-during-stream integration", () => {
	it("keeps the viewport scrolled up while a response streams, and resumes following once scrolled back to bottom", async () => {
		...
		// short-lived frame. Raw-stream matching cannot help here -- while scrolled
		// away from the streaming tail the draft rows are never painted, so chunk
		// text never enters the byte stream.
